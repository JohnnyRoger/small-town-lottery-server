...
router.post("/", function (req, res, next) {
    ...
    pool.query("SELECT t1.drawdate 'DATE', t4.agentcode 'AGENT', t5.loccode 'LOCATION', COALESCE(t6.winningno,'TBA') 'result'," +
        " (SELECT SUM(e1.amount) FROM betdetails e1 INNER JOIN betheader f1 ON e1.betheaderserial_pk = f1.betheaderserial_pk " +
        " WHERE f1.drawdate = t1.drawdate AND f1.agentserial_pk = t1.agentserial_pk) 'totalBet', " +
        " COALESCE((SELECT SUM(e1.win) FROM betdetails e1 LEFT JOIN betheader f1 ON e1.betheaderserial_pk = f1.betheaderserial_pk" +
        " WHERE e1.isrambolito = 0 AND e1.betno = t6.winningno AND f1.drawdate = t1.drawdate AND f1.agentserial_pk = t1.agentserial_pk GROUP BY f1.transcode),0.00) " +
        " + " +
        " COALESCE((SELECT SUM(e1.win) FROM betdetails e1 LEFT JOIN betheader f1 ON e1.betheaderserial_pk = f1.betheaderserial_pk" +
        " WHERE e1.isrambolito = 1 AND e1.betno LIKE CONCAT('%',LEFT(t6.winningno,1),'%') AND f1.drawdate = t1.drawdate AND f1.agentserial_pk = t1.agentserial_pk " +
        ...
        " SELECT SUM(e1.win) FROM betdetails e1 LEFT JOIN betheader f1 ON e1.betheaderserial_pk = f1.betheaderserial_pk WHERE e1.isrambolito = 2 AND f1.drawdate = t1.drawdate AND f1.agentserial_pk = t1.agentserial_pk AND" +
        ...
        " WHERE t1.drawdate = ? AND t1.agentserial_pk = ?" +
        " GROUP BY t1.drawdate",
        [req.query.date, req.query.agent],
